...
SoundEngineModule.handleBrowserEvent
  ...
  │  • theme manager resolves event id → sound URL
  ▼
...
3. Priority preemption  — global cooldown bypassed because arriving > in-flight priority
```
...
3. Tier-based fallback (manifest.fallbacks.tier1 / tier2 / tier3)
